...
async def get_user_orders(userId: str, role: str):
    db = get_db()
    if role == "farmer" or role == "trader":
        # Get orders where user sold something (we look at order_items)
        items_res = db.table('order_items').select('orderId').eq('farmerId', userId).execute()
        order_ids = [item['orderId'] for item in items_res.data]
        if not order_ids:
            return []
        # In Supabase, using 'in' filter is possible
        orders_res = db.table('orders').select('*, order_items(*)').in_('id', order_ids).order('createdAt', desc=True).execute()
        return orders_res.data
    else:
        # Get orders where user is buyer
        orders_res = db.table('orders').select('*, order_items(*)').eq('buyerId', userId).order('createdAt', desc=True).execute()
        return orders_res.data
